...
#define SOCKETTEST_IP     "192.168.1.24"
...
Void task1_func(UArg arg0, UArg arg1)
{
    ...
    IIC_OpenComm();
    while(1){
        Semaphore_pend(semaphore0, BIOS_WAIT_FOREVER); //posted in Hwi with period 1 sec.
        setCalibration();
        setPressure();
...
Void task2_func(UArg arg0, UArg arg1){
    ...
    while(1){
        ...
        System_printf("Pressure and changing on coordinates are measured and average them. \n");
        System_flush();
        if(abs(average) < 644,7){ //644,7 calculated by formula in BMP180 datasheet page 16
                                   //if the pressure value less than 644,7 it mean falling more than 3 meter for Eskisehir
            boolean = 1;
...
Void task_getMove(UArg arg0, UArg arg1){
    int boolean;
    int x,y,z;
    int Average_Move;
    while(1){
        ...
        if(boolean){
            ...
            MoveZ = MoveZ + abs(z);
            if(ctr_Move % 5 == 0){
                Average_Move = abs(abs(MoveX + MoveY + MoveZ)/5 - (abs(x) + abs(y) + abs(z)));
                System_printf("\n MoveX + MoveY + MoveZ /5 - (abs(x) + abs(y) + abs(z)) %d\n", Average_Move);

                if(Average_Move < 10){      // if the axis not change then object is not moving
                    System_printf("The falling object is not moving, post true with mailbox. \n", average);
                    System_flush();
                    boolean = 1;
                    ...
                    System_printf("The falling object is moving, post true with mailbox. \n", average);
                    System_flush();
                    boolean = 0;
                    Mailbox_post(mailbox2, &boolean, BIOS_NO_WAIT);
                    MoveX = 0;
...
Void task3_func(UArg arg0, UArg arg1){
    char data[30] = "FreeFall is detected.";
    int boolean;
    while(1){
